            ...
            <van-tab title="医生诊断">
                <div class="add-btn">
                    <van-button
                        round
                        type="info"
                        :to="`/supervisor/add-diagnostics?pId=${pId}`"
                        >添加诊断</van-button
                    >
            ...
            <van-tab title="就诊记录">
                <div class="add-btn">
                    <van-button
                        round
                        type="info"
                        :to="`/supervisor/add-record?pId=${pId}`"
                        >添加就诊记录</van-button
                    >
                </div>
...
export default {
    ...
    methods: {
        getInfo() {
            this.$api
                .get(`/qkys/api/sup/getPatientHealthRecordByPId/${this.pId}`)
                .then(res => {
                    const { data } = res;
        ...
        submit() {
            this.$api
                .post(`/qkys/api/sup/updatePatientHealthCondition`, {
        ...
        getDocDiagnosis() {
            this.$api
                .get(`/qkys/api/sup/getDiagnosticaByPId/${this.pId}`)
                .then(res => {
                    const { data } = res;
        ...
        delDiagnostics(data) {
            Dialog.confirm({
                ...
                .then(() => {
                    this.$api
                        .get(`/qkys/api/sup/deleteDiagnosticaById/${data.id}`)
                        .then(() => {
